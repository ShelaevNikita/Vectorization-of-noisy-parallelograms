...
#include "Vectorization_parallelograms.h"
    ...
    void Vectorization_parallelograms::find_min_MSD(line_result (&res), const vector<pair<double, double>> &array_of_points) {
    ...
    double Vectorization_parallelograms::foo_fmin(bool &flag, double &k, double sum, double b, vector<pair<double, double>> points) {
    ...
    double
    Vectorization_parallelograms::f_min(bool &flag, double bmin, double bmax, double &k, double &b, const vector<pair<double, double>> &points) {
        double sum = 0.0;
        for (auto &point : points)
            sum += pow(point.first, 2);
        double Lcenter, Rcenter, Lfcenter, Rfcenter, beg, end, BegBuf, EndBuf;
        double delta = (sqrt(5) - 1) / 2;
        beg = bmin;
        end = bmax;
        foo_fmin(flag, k, sum, beg, points);
        foo_fmin(flag, k, sum, end, points);
    ...
    double Vectorization_parallelograms::rhoStepEstimation(vector<pair<float, float>> *points) {
