...
private:
    ...
    double error_MSD;
    int max_counter;
    double error_quadrangle;
    double d;

public:

    MSD(double error_fmin_Init, double error_MSD_Init, int max_counter_Init, double error_quadrangle_Init, double d_Init)
    {
        error_fmin = error_fmin_Init;
        error_MSD = error_MSD_Init;
        max_counter = max_counter_Init;
        error_quadrangle = error_quadrangle_Init;
        d = d_Init;
    }
...
private:
    ...
    void comparison(double res_para[4][3], double res_mono[4][3], int para[2][2], 
        double result_para[4][2], double result_mono[4][2], double (&result)[4][2])
    {
        int counter = 0;
        for (int i = 0; i <= 1; i++)
        {
 ...
 public:
    ...
    void MSD_main(vector<vector<pair<double, double>>> Points, vector<pair<double, double>> Lines, double(&result)[4][2])
    {
        ...
        find_min_MSD_para(res_para, squad_of_pairs[0], Points);
        find_min_MSD_para(res_para, squad_of_pairs[1], Points);
        for (int i = 0; i <= 3; i++) find_min_MSD_mono(res_mono, i, Points);
        ...
        comparison(res_para, res_mono, squad_of_pairs, result_para, result_mono, result);
    }
};